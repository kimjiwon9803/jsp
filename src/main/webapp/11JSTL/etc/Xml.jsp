...
<!-- XML 파일을 위해 parse태그에 1차로 설정한다. 뒤에서 blist라는
변수명으로 각 노드를 파싱할 것이다. -->
    <x:parse xml="${booklist}" var="blist" />
    
   <!-- bookilist 하위 노드에는 2개의 book이 있으므로 인덱스를 통해 접근한 후
   데이터를 파싱한다.
   이와같이 xml문서를 마치 경ㄹ오처럼 접근하는 것을 XPath라고 한다.
   XPath는 EL과는 다르게 {} (중괄호)를 사용하지 않는다.-->
    ...
    <table border="1">
        ...
        <!-- 해당 노드에는 2권의 책 정보고 있으므로 반복할 수 있다.-->
