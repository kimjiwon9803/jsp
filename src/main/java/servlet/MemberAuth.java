...
public class MemberAuth extends HttpServlet {
	...
	/*클라이언트가 최초로 요청했을떄 서블릿 객체가 생성되는데 이떄
	 * 최초 1번만 호출되는 init() 메서드에서 DB연결 처리를 한다.*/
	public void init() throws ServletException{
		/*서블릿 내에서 application 내장객체를 얻어온다.
		 * 모델2 방식에서는 서블릿이 먼저 요청을 받기 떄문에 모델1 방식과 같이 jsp에서 매개변수로
		 * 내장객체를 전달할 수 없다. 따라서 각 내장객체를 얻어올 수있는
		 * 메서드가 존재한다.*/
	...
	/*service() 매서드는 get/post 방식 둘다 요청을 받을 수 있다.*/
	@Override
	protected void service(HttpServletRequest req, HttpServletResponse resp)
		 ...
		 * 회원인즈응ㄹ 진행한 후 일치하는 정보가 있다면 회원레코드르 DTO에 저장 후
		 * 반환한다.*/
		MemberDTO memberDTO = dao.getMemberDTO(id, pass);
		
		String memberName= memberDTO.getName();
		if(memberName != null) {
			/*DTO 객체에 회원이 이름이 저장되어 있다면 로그인에 성공한 경우로 
			 * 판단할 수 있다.*/
